...
class atlassian_jira_report:
    ...
    def __init__(self, project, report_name):
        ...
        self.output_obj     = jira(project=self.project, name=self.output_name)
        self.progress       = 0
    ...
    def generate_report(self, current_run_id, baseline_run_id = None):
            ...
            screenshots = self.grafana_obj.render_image(self.graphs, self.current_start_timestamp, self.current_end_timestamp, self.test_name, current_run_id)
            self.status = "Rendered images in Grafana"
            self.progress = 50

            response = self.output_obj.put_page_to_jira(title=jiraPageName)

            for screenshot in screenshots:
                fileName = self.output_obj.put_image_to_jira(issue=response, image=screenshot["image"], filename=screenshot["name"])
            ...
            self.output_obj.update_jira_page(jiraissue=response,description=body)
